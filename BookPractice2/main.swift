...
let duration = Float64(22)

// MARK: - Checks if input is well formated
precondition(CommandLine.arguments.count > 2,"Error >> You must provide a frequency and a scale pattern in binary")
...
let scalePattern = CommandLine.arguments[2]
do{
    ...
    print("Log >> Generating at \(frequency)hz with \(CommandLine.arguments[2]) pattern")
